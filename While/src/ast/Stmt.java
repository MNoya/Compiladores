 ...
 * construcciones del lenguaje que modifican (potencialmente) los valores de las
 * variables en el estado del programa.
 */
public abstract class Stmt{
    ...
    public static Stmt generate(Random random, int min, int max) {
        ...
        switch (i) {
            ...
            //Non terminals
            case 0:
                return Assignment.generate(random, min - 1, max - 1);
            case 1:
                return Sequence.generate(random, min - 1, max - 1);
            case 2:
                return IfThenElse.generate(random, min - 1, max - 1);
            case 3:
                return WhileDo.generate(random, min - 1, max - 1);
            default:
